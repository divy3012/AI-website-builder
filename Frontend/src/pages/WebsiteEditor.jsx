...
function WebsiteEditor() {
  ...
  useEffect(() => {
    const handleGetWeb = async () => {
      try {
        const result = await axios.get(
          `${serverUrl}/api/website/getwebsite/${id}`,
          { withCredentials: true },
        );
    ...
    <div className="h-screen w-screen flex bg-black text-white overflow-hidden">
        ...
        <div className="flex flex-col h-full">
          <div className="flex-1  overflow-y-auto px-4 py-4 space-y-4">
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${m.role === "user" ? "bg-white text-black text-sm" : "bg-white/5 border border-white/10 text-zinc-200"} `}
                >
                  {m.content}
                </div>
              </div>
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-14 px-4 flex items-center justify-between border-b border-white/10 bg-black/80">
          ...
          >
            <ArrowLeft size={16} /> <span> Live Preview</span>
          </div>
          <div className="flex gap-2">
            {website.deployed ? (
              ""
              ...
              <button
                className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-linear-to-r from-indigo-400 to-purple-500 text-sm font-semibold hover:scale-105 transition"
  ...
  function Header({ onclose }) {
    return (
      <div className="h-14 px-4 flex items-center justify-between border-b border-white/19">
